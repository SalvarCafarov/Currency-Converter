...
const numberMaskTo = IMask(
    document.querySelector('input[name=to]'), {
        mask: Number,
        radix: '.',
        mapToRadix: [','],
    });
const numberMaskFrom = IMask(
...
async function getData(e) {
    ...
    if (e.target.name !== 'to') {
        ...
        if (data.from != '') {
            data.to = (data.from * (rate).toFixed(2)).toFixed(2)
        } else {
            data.to = '';
        }
        ...
        const toValue = document.querySelector('input[name=from]');
        if (data.to != '') {
            data.from = (data.to * (rate).toFixed(2)).toFixed(2)
        } else {
            data.from = '';
        }
        toValue.value = data.from;
    }
}
